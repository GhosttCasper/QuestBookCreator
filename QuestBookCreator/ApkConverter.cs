...
namespace QuestBookCreator
{
    class ApkConverter
    {
        public List<string> get_processed(Node curN, string pathToAssets)
        {
            string text = curN.getContent().getInfo();
            List<string> list = new List<string>();
            int countForImageNaming = 0;
            int link = 0;
            while (true)
            {
                ...
                if (text.IndexOf("image:") == 2) //изображение
                {
                    ...
                    string imageName = curN.get_name() + "-" + countForImageNaming + ".jpg";
                    countForImageNaming++;
                    string fullPathDest = pathToAssets + "\\" + imageName;
                    System.IO.File.Copy(path, fullPathDest, true);
                    //string pathForHtml = "pics/" + imageName;
                    list.Add("<img src=\"" + imageName + "\">");
                }
